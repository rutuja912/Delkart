...
function IssuesForm() {
  ...
  const getCompleteOrder = () => {
    axios
      .get(`http://localhost:8070/stock/request/${id}`)
      .then((res) => {
  ...
  const getStock = async () => {
    axios
      .get(`http://localhost:8070/stock/ViewStockname/${name}`)
      .then((res) => {
  ...
  stockUtil.map((data) => {
    if (
      ...
      data.firstPurchaseDate === compareDate
    ) {
      if (data.type === 'Additions') totAdds += data.quantity;
      if (data.type === 'Issues') totIssues += data.quantity;
    }
  });
                ...
                <div className="m-2 md:m-10 mt-24 p-2 md:p-10 bg-white rounded-3xl  dark:bg-secondary-dark-bg dark:text-white ">
                  ...
                  <div className=" flex items-center justify-center ">
                    <form
                      onSubmit={async (e) => {
                        ...
                        if (remaining >= quantity) {
                          ...
                          await axios
                            .put(
                              'http://localhost:8070/stock/updateStatus/' + id,
                              { status: salesStatus }
                            )
                            .then((res) => {})
                            .catch((error) => {
                    ...
                    >
                      ...
                      <div className="mb-3">
                        <label for="additions" className="form-label">
                          Type:{' '}
                        </label>
                        <input
                          type="text"
