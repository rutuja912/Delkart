...
const MaintenanceDashboard = () => {
  ...
  const maintprog = maintainence.filter(
    (maint) => maint.status === 'In progress'
  ).length;
  const maintainenceVehip = maintainenceVehi.filter(
    (maint) => maint.status === 'In progress'
  ).length;
  const maintainenceMachinep = maintainenceMachine.filter(
    (maint) => maint.status === 'In progress'
  ).length;
                  ...
                  <div className="flex flex-wrap lg:flex-nowrap justify-center mt-5">
                    <div className="flex m-3 flex-wrap justify-center gap-1 items-center">
                      ...
                      <DashTopBox
                        ...
                        label="Total Maintenances Records"
                        data={maintprog + machineMaintCount + vehiMaintCount}
                      />
                      <DashTopBox
                        icon={<GiMoneyStack />}
