...
const navigate = useNavigate();
  ...
  const countPresent = attendance.filter(
    (att) =>
      new Date(att.employeeInTime).toISOString().slice(0, 10) ===
      date.slice(0, 10)
  ).length;
  const countAbsent = empCount - countPresent;
  const countPending = leave.filter(
    (leav) => leav.leaveStatus === 'Pending'
  ).length;
                  ...
                  >
                    <table className="w-full rounded-lg">
                      ...
                      <tbody>
                        {attendance
                          .filter((data) => {
                            const date = new Date(data.employeeInTime)
                              .toISOString()
                              .split('T')[0];
                            if (
                  ...
                  >
                    <table className="w-full rounded-lg">
                      ...
                      <tbody>
                        {leave
                          .filter((data) => {
                            const date = new Date(data.leaveStartDate)
                              .toISOString()
                              .split('T')[0];
                            if (
                              date == new Date().toISOString().split('T')[0]
                          ...
                          .map((data, key) => {
                            ...
                            return (
                              ...
                              >
                                <TableData value={data.employeeNumber} />
                                <TableData
                                  value={data.employeeDetails.map((data3) => {
                                    return <div>{data3.employeeFullName}</div>;
                                  })}
                                />
                                <TableData value={data.leaveType} />
                                <TableData
                                  value={
                                    new Date(data.leaveStartDate)
                                      .toISOString()
                                      .split('T')[0]
                                  }
                                />
