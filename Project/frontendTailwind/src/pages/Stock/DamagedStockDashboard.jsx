...
const DamagedStockDashboard = () => {
  ...
  const getdamagedStock = async () => {
    ...
    axios
      .get('http://localhost:8070/damagedStock/')
      .then((res) => {
  ...
  useEffect(() => {
    //useEffect is used to call the function getdamagedStock
    getdamagedStock();
  }, []);

  const itemCount = damagedStock.length;
  const countRawMaterials = damagedStock.filter(
    (dstk) => dstk.damagedStockCategory === 'Raw materials'
  ).length;
  const countWorkInProgress = damagedStock.filter(
    (dstk) => dstk.damagedStockCategory === 'Work in progress'
  ).length;
  const countFinishedGoods = damagedStock.filter(
    (dstk) => dstk.damagedStockCategory === 'Finished goods'
  ).length;
      ...
      <div className="flex flex-wrap lg:flex-nowrap justify-center mt-5">
        <div className="flex m-3 flex-wrap justify-center gap-1 items-center">
          ...
          <DashTopBox
            ...
            data={countRawMaterials}
          />
          <DashTopBox
            icon={<GiSewingNeedle />}
            label="Total Work in progress"
            data={countWorkInProgress}
          />
          <DashTopBox
            icon={<GiClothes />}
            label="Total Finished Goods"
            data={countFinishedGoods}
          />
        </div>
      </div>
