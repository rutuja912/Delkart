...
const IncomeStatement = () => {
  ...
  const getFinanceDate = async () => {
    axios
      .get(
        'http://localhost:8070/finance/date/' +
          location.state.DS +
          '/' +
          location.state.DE
      )
  ...
  const getTransport = async () => {
    axios
      .get(
        'http://localhost:8070/transport/date/' +
          location.state.DS +
          '/' +
          location.state.DE
      )
  ...
  for (var i = 0; i < salary.length; i++) {
    salaryTotal +=
      salary[i].employeeIncentive +
      salary[i].employeeAllowance +
      salary[i].employeeBasicSalary;
  }
  ...
  var grossProfit = formatter.format(
    saleTotal - productionTotal - purchasesTotal
  );
  ...
  var maintainanceTotal = formatter.format(
    maintTotal + MMaintTotal + VehiMaintTotal
  );

  const toGeneratePDF = () => {
    navigate('/IncomeStatementDateRangePreview', {
                ...
                <div>
                  <div className="m-2 md:m-10 mt-24 p-2 md:p-10 bg-white rounded-3xl dark:bg-secondary-dark-bg dark:text-white">
                    <Header
                      category="Table"
                      title={
                        `Income Statement from ` + dateStart + ` to ` + dateEnd
                      }
                    />
                      ...
                      <table className="w-full rounded-lg">
                        <thead>
                          <tr className="bg-slate-200 text-md h-12 dark:bg-slate-800 dark:text-white">
                            <td className="px-4 py-3 text-md whitespace-nowrap font-semibold text-left text-black-300">
                              Description
                            </td>
                            <td className="px-4 py-3 text-md whitespace-nowrap font-semibold text-right text-black-300">
                              Amount
                            </td>
                          </tr>
                        </thead>
                        <tbody>
                          <tr className="text-sm h-10 border dark:border-slate-600 dark:text-white">
                            ...
                            <td className=" text-right px-3 align-middle border-l-0 border-r-0 text-m whitespace-nowrap p-3">
                              {saleTotal}
                            </td>
                          </tr>
                          <tr className="text-sm h-10 border dark:border-slate-600 dark:text-white">
