...
const StockUtilPieChart = () => {
  ...
  const processing = pendingStock.filter(
    (stk) => stk.status === 'Processing'
  ).length;
  const resolved = pendingStock.filter(
    (stk) => stk.status === 'Resolved'
  ).length;
  ...
  return (
    <div>
      <ChartsHeader
        category="Chart"
        title="Stock Utilisation Distribution By Requests"
      />
      <AccumulationChartComponent
        ...
        <AccumulationSeriesCollectionDirective>
          <AccumulationSeriesDirective
            ...
            dataSource={[
              {
                x: 'Processing',
                y: ((processing / count) * 100).toPrecision(4),
                text: ((processing / count) * 100).toPrecision(2) + '%',
              },
              {
                x: 'Resolved',
                y: ((resolved / count) * 100).toPrecision(4),
                text: ((resolved / count) * 100).toPrecision(2) + '%',
              },
            ]}
            xName="x"
            yName="y"
