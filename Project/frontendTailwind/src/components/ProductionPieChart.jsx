...
const ProductionPieChart = () => {
  ...
  const prodCollars = Order.filter(
    (Order) => Order.status == 'Stock Requested'
  ).length;
  const prodTrousers = Order.filter(
        ...
        <AccumulationSeriesCollectionDirective>
          <AccumulationSeriesDirective
            ...
            dataSource={[
              {
                x: 'Pending',
                y: ((prodT / prodCount) * 100).toPrecision(4),
                text: ((prodT / prodCount) * 100).toPrecision(2) + '%',
              },
              {
                x: 'Stock Requested',
                y: ((prodCollars / prodCount) * 100).toPrecision(4),
                text: ((prodCollars / prodCount) * 100).toPrecision(2) + '%',
              },
              {
                x: 'Completed',
                y: ((prodTrousers / prodCount) * 100).toPrecision(4),
                text: ((prodTrousers / prodCount) * 100).toPrecision(2) + '%',
              },
              {
                x: 'Costed',
                y: ((prodShirts / prodCount) * 100).toPrecision(4),
                text: ((prodShirts / prodCount) * 100).toPrecision(2) + '%',
              },
            ]}
            xName="x"
            yName="y"
