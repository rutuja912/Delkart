...
const DamagedStockPieChart = () => {
  ...
  const dsUsables = damagedStock.filter(
    (ds) => ds.usability === 'Usable'
  ).length;
  const dsNotUsable = damagedStock.filter(
    (ds) => ds.usability === 'Not usable'
  ).length;
        ...
        <AccumulationSeriesCollectionDirective>
          <AccumulationSeriesDirective
            ...
            dataSource={[
              {
                x: 'Usable',
                y: ((dsUsables / itemCount) * 100).toPrecision(4),
                text: ((dsUsables / itemCount) * 100).toPrecision(2) + '%',
              },
              {
                x: 'Not Usable',
                y: ((dsNotUsable / itemCount) * 100).toPrecision(4),
                text: ((dsNotUsable / itemCount) * 100).toPrecision(2) + '%',
              },
            ]}
            xName="x"
            yName="y"
