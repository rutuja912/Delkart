...
const PurchaseOrderStatusPieChart = () => {
  ...
  const placedOrders = purchaseOrder.filter(
    (order) => order.orderStatus === 'Order Placed'
  ).length;
  const completedOrders = purchaseOrder.filter(
        ...
        <AccumulationSeriesCollectionDirective>
          <AccumulationSeriesDirective
            ...
            dataSource={[
              {
                x: 'Placed Orders',
                y: ((placedOrders / purOrder) * 100).toPrecision(4),
                text: placedOrders + ' ' + 'Orders',
                color: '#419D78',
              },
              {
                x: 'Completed Orders',
                y: ((completedOrders / purOrder) * 100).toPrecision(4),
