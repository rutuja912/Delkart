...
const AttendancePieChart = () => {
  ...
  const countPresent = attendance.filter(
    (att) =>
      new Date(att.employeeInTime).toISOString().split('T')[0] ===
        ...
        <AccumulationSeriesCollectionDirective>
          <AccumulationSeriesDirective
            ...
            dataSource={[
              {
                x: 'Present',
                y: ((countPresent / empCount) * 100).toPrecision(4),
                text: countPresent + ' ' + 'Employees',
                color: '#419D78',
              },
              {
                x: 'Absent',
                y: ((countAbsent / empCount) * 100).toPrecision(4),
                text: countAbsent + ' ' + 'Employees',
                color: '#C14953',
              },
            ]}
